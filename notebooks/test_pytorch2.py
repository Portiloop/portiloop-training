...
class TransformerEncoderWithCLS(nn.Module):
    def __init__(self, embedder, embedding_size, num_heads, num_layers, num_classes):
        ...
        self.positional_embedding = nn.Embedding(10 * 250, embedding_size).to(device) # Positional embedding
    ...
    def forward(self, x):
        ...
        positions = self.positional_embedding(positions).expand(batch_size, seq_len, embedding_size)
        x = x + positions
