...
from portiloop_ml.portiloop_python.Utils.utils import sample_config_dict
...
meta_model, meta_loss = train_surrogate(
    meta_model, deepcopy(finished_experiment))
...
print(
    f"DEBUG: test model loss = {test_model_loss} / real loss = {test['cost_software']}")
test = test_network2
print(test)
with torch.no_grad():
    config_dict = test["config_dict"]
...
print(
    f"DEBUG: test model loss = {test_model_loss} / real loss = {test['cost_software']}")
test, _ = sample_config_dict(
    name="test2", previous_exp={}, all_exp=finished_experiment)
# print(test)
with torch.no_grad():
    config_dict = test
    x = transform_config_dict_to_input(config_dict)
