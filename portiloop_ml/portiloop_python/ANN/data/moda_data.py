...
def generate_dataloader(config):
    all_subject = pd.read_csv(Path(
        config['path_dataset']) / config['subject_list'], header=None, delim_whitespace=True).to_numpy()
    test_subject = None
    if config['phase'] == 'full':
        p1_subject = pd.read_csv(Path(
            config['path_dataset']) / config['subject_list_p1'], header=None, delim_whitespace=True).to_numpy()
        p2_subject = pd.read_csv(Path(
            config['path_dataset']) / config['subject_list_p2'], header=None, delim_whitespace=True).to_numpy()
        train_subject_p1, validation_subject_p1 = train_test_split(
            p1_subject, train_size=0.8, random_state=config['split_idx'])
        if config['test_set']:
            test_subject_p1, validation_subject_p1 = train_test_split(
                validation_subject_p1, train_size=0.5, random_state=config['split_idx'])
        train_subject_p2, validation_subject_p2 = train_test_split(
            p2_subject, train_size=0.8, random_state=config['split_idx'])
        if config['test_set']:
            test_subject_p2, validation_subject_p2 = train_test_split(
                validation_subject_p2, train_size=0.5, random_state=config['split_idx'])
        ...
        train_subject, validation_subject = train_test_split(
            all_subject, train_size=0.8, random_state=config['split_idx'])
        if config['test_set']:
            test_subject, validation_subject = train_test_split(
                validation_subject, train_size=0.5, random_state=config['split_idx'])
    ...
    if config['seq_len'] is not None:
        ...
        batch_size_validation = len(list(range(0, (config['seq_stride'] // config['validation_network_stride'])
                                    * config['validation_network_stride'], config['validation_network_stride']))) * nb_segment_validation
        ...
        ds_validation = SignalDataset(filename=filename,
                                      ...
                                      len_segment=len_segment,
                                      threshold=config['threshold'])
        idx_true, idx_false = get_class_idxs(
            ds_train, config['distribution_mode'])
        ...
        print(
            f"DEBUG: Validation dataloader batch size: {batch_size_validation}")
        ...
        batch_size_test = len(list(range(0, (config['seq_stride'] // config['validation_network_stride'])
                              * config['validation_network_stride'], config['validation_network_stride']))) * nb_segment_test
