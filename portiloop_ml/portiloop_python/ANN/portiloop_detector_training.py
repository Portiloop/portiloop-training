...
def run_inference(dataloader, criterion, net, device, hidden_size, nb_rnn_layers, classification, batch_size_validation, threshold, out_features, recurrent=True):
    ...
    if recurrent:
        h1 = torch.zeros(
            (nb_rnn_layers, batch_size_validation, hidden_size), device=device)
    ...
    for epoch in range(first_epoch, first_epoch + nb_epoch_max):
        ...
        if epoch > -1:
            ...
            for batch_data in train_loader:
                ...
                if n % 100 == 0:
                    print(
                        f"batch {n} / {nb_batch_per_epoch} in {end - start} s")
                n += 1
                start = time.time()
            _t_stop = time.time()
...
def run(config_dict, wandb_project, save_model, unique_name, wandb_group):
    ...
    train_loader_mass, validation_loader_mass = get_dataloaders_mass(
        config_dict)
    MASS = True
    if MASS:
